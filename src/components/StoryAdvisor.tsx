...
export default function StoryAdvisor({ isOpen, onClose, onTalkToCrossMedia }: StoryAdvisorProps) {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([
        {
          role: "model",
          text: `Every organisation has a story. Tell me a little about yours and I'll help you discover what makes it worth telling. (v1.2.4 - High Speed Connection) \n\nWhat does your organisation do, and who does it serve?`
        }
      ]);
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // v1.1.8: Verified Model Selection from Scan
      const modelConfigs = [
